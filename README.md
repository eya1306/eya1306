# hi, i'm eya thebty 👋
**Full-stack Developer** • Laravel • React • PHP • JavaScript  
📍 Sousse, Tunisia · she/her

---

## 🔭 About me
I build web apps and small products. Currently learning Symfony and improving my backend testing skills. I enjoy turning design into clean, maintainable code and automating workflows.

## 🧰 Tech stack
- **Backend:** PHP (Laravel, Symfony), Node.js, MySQL
- **Frontend:** HTML5, CSS3, Bootstrap, React, jQuery
- **Tools:** Git, Docker, Composer, npm
- **Other:** WordPress, REST APIs, GitLab CI

## 🚀 Featured projects
- **[Resiwave](https://github.com/eya1306/Resiwave)** — Platform for syndic managers (payments & communication). `#laravel #php`
- **[OG_store](https://github.com/eya1306/OG_store)** — Commercial website demo with product catalog and checkout. `#ecommerce`
- **[PRODIGY_FS_01](https://github.com/eya1306/PRODIGY_FS_01)** — Internship project (backend + API).
*(see pinned repos on my profile for more)*

## 📈 GitHub stats
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=eya1306&layout=compact)
![Stats](https://github-readme-stats.vercel.app/api?username=eya1306&show_icons=true&count_private=true)

## 📫 Contact
- Email: ayathebty13@gmail.com
- LinkedIn: https://www.linkedin.com/in/eya-thebty-553004238/
- phone: +21653228647

---

## ✨ How you can help me
- Star/fork a repo you like
- Open issues if you find bugs
- Follow if you want to see new web projects

---


